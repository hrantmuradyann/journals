...
| `templates/index.html`, `static/style.css` | The page (Neovim-dark colour scheme — palette variables at the top of `style.css` — monospace, no title header). Vanilla JS, no build step. Tabs / search / publication chips are all client-side filters over the one `/articles` payload. Two extra views sit behind the header buttons: **Read** (rating filter + sort) and **Calendar** (month grid built from `read_at` in the reading history; each day is shaded by how many articles were read, click a day for its articles). |
